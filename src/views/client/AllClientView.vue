...
<template>
  ...
  <UserLayout>
    <div class="flex justify-between items-center pr-[50px] pl-10">
      <div class="text-3xl font-bold  ml-[30px] mt-[10px]">All Clients</div>
      <!-- <div class="flex-none gap-2 mt-[15px] ml-[80px]">
      ...
      <div class="flex">
        <div class="flex-none gap-2 mt-[15px] mr-[20px]">
          <div class="form-control w-[300px] h-[40px] ">
            <input v-model="search" type="text" placeholder="Search"
              class="input input-bordered w-24 md:w-auto border-2 rounded-xl" />
          </div>
        </div>
        <RouterLink :to="{ name: 'add-new-client' }"
          class=" flex justify-center items-center bg-[#0000ffca] text-white capitalize hover:bg-[#0000ffa7] mt-[14px] w-[80px] h-[40px] rounded-xl font-semibold">Add new
        </RouterLink>
      </div>
    </div>
          ...
          <tr v-for="(client, index) in clientStore.clientFilter" :key="client.id">
            ...
            <td>
              ...
              <RouterLink :to="{ name: 'update-client', params: { id: client.id } }"
                class=" p-[10px] pl-[15px] pr-[15px] ml-[8px] text-white bg-[#008000af] font-semibold rounded-xl hover:bg-[#00800052] ">
                Edit
                <!-- <img :src="Edit" class="w-6 h-6 mt-[20px]" /> -->
              </RouterLink>
            </td>
            <!-- <td>
              <RouterLink :to="{ name: 'update-client', params: { id: client.id } }"
                class=" btn bg-[#008000af] mr-11 text-[white] hover:bg-[#00800052] ">
                Edit
              </RouterLink>
            </td> -->
          </tr>
